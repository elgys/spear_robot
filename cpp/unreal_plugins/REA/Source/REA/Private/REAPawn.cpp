...
#include "REA/REAWheel.h"
#include "REA/REAsphereWheel.h"

BEGIN_SUPPRESS_COMPILER_WARNINGS
AREAPawn::AREAPawn(const FObjectInitializer& object_initializer) : APawn(object_initializer)
{
    std::cout << "[SPEAR | REAPawn.cpp] AREAPawn::AREAPawn" << std::endl;
    ...
    UClass* wheel_class = UREAWheel::StaticClass();
    UClass* sphere_class = UREAsphereWheel::StaticClass();
    ...
    vehicle_movement_component_->WheelSetups[2].WheelClass = sphere_class;
    ...
    vehicle_movement_component_->WheelSetups[3].WheelClass = sphere_class;
...
void AREAPawn::SetupPlayerInputComponent(UInputComponent* input_component)
{
    ASSERT(input_component);
    APawn::SetupPlayerInputComponent(input_component);

    input_component->BindAxis("MoveForward", this, &AREAPawn::moveForward);
    input_component->BindAxis("MoveRight", this, &AREAPawn::moveRight);
}

void AREAPawn::Tick(float delta_time)
{
...
BEGIN_SUPPRESS_COMPILER_WARNINGS
void AREAPawn::setBrakeTorques(const Eigen::Vector4f& brake_torques)
{
...
Eigen::Vector4f AREAPawn::getDutyCycle() const
{
    return duty_cycle_;
}

void AREAPawn::setDutyCycle(const Eigen::Vector4f& duty_cycle)
{
    ...
    // First make sure the duty cycle is not getting above 100%. This is done similarly on the real REA.
    // https://github.com/isl-org/REA/blob/master/android/app/src/main/java/org/REA/vehicle/Control.java

    duty_cycle_ = duty_cycle.cwiseMin(1.f).cwiseMax(-1.f);
}

BEGIN_SUPPRESS_COMPILER_WARNINGS
Eigen::Vector4f AREAPawn::getWheelRotationSpeeds() const
{
...
void AREAPawn::moveForward(float forward)
{
...
void AREAPawn::moveRight(float right)
{
...
BEGIN_SUPPRESS_COMPILER_WARNINGS
void AREAPawn::setDriveTorquesFromDutyCycle()
{
...
Eigen::VectorXf AREAPawn::rpmToRadSec(Eigen::VectorXf rpm)
{
    return rpm * PI / 30.f;
}

// rad/s to rev per minute
Eigen::VectorXf AREAPawn::radSecToRpm(Eigen::VectorXf omega)
{
